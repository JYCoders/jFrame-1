...
define(function (require) {
    ...
    var zrGuid = require('zrender/tool/guid');
    ...
    Force.prototype = {
        ...
        /// <summary>初始化元素布局</summary>
        /// <param name="serie">图表生成的数据内容数组</param>
        _initLayout: function(serie) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (typeof(gNode.data.initial) !== 'undefined') {
                    gNode.layout.position = vec2.create();
                    vec2.copy(gNode.layout.position, gNode.data.initial);
                } else if (typeof(this.__nodePositionMap[gNode.id]) !== 'undefined') {
                    gNode.layout.position = vec2.create();
        ...
        /// <param name="serie">图表生成的数据内容数组</param>
        /// <param name="serieIdx">数据内容数组序号</param>
        _buildNodeShapes: function(serie, serieIdx) {
            ...
            graph.eachNode(function (node) {
                ...
                // wlg 2015-02-05 START
                /* 不需要设置，_initLayout方法中会设置节点的位置信息
                var center = this._layout.center;
                var size = Math.min(this._layout.width, this._layout.height);
                var random = _randomInSquare(
                		center[0], center[1], size*0.8
                    );
                x = typeof(node.data.initial) === 'undefined' 
                    ? random.x
                    : node.data.initial.x;
                y = typeof(node.data.initial) === 'undefined'
                    ? random.y
                    : node.data.initial.y;
                */    
                ...
                if (shape.style.iconType == "rectangle") {
                	shape.style.width = (this.deepQuery(queryTarget, 'symbolWidthSize') || 0) * 1;
                    shape.style.height = (this.deepQuery(queryTarget, 'symbolHeightSize') || 0) * 1;
                } else {
                	// 长宽相等，圆形或正方形
                ...
                if (this.deepQuery(queryTarget, 'itemStyle.normal.label.show')) {
                	if (node.data.label != null && node.data.label.length > 0) {
                		shape.style.text = node.data.label;
                	} else {
                ...
                // wlg 2015-02-05 START
                //自定义事件
                if(typeof(node.data.ondblclick) !== 'undefined'){
                    if(node.data.ondblclick) {
                        shape.ondblclick = node.data.ondblclick;
                    }
                }
                if(typeof(node.data.onclick) !== 'undefined'){
                    if(node.data.onclick) {
                        shape.onclick = node.data.onclick;
                    }
                }
                // wlg 2015-02-05 END
        ...
        _buildLinkShapes: function(serie, serieIdx) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (serie.linkSymbol && serie.linkSymbol !== 'none') {
                    // linkType = 'line';
                    linkType = serie.linkSymbol;
                }
                //var LinkShapeCtor = linkType === 'line' ? LineShape : BrokenLine;
				var linkShape;
				if (linkType == 'line') {
					linkShape = new LineShape({
	                    style : {
	                        xStart : 0,
	                        yStart : 0,
	                        xEnd : 0,
				...
				} else if (linkType == 'brokenLine') {
					linkShape = new BrokenLine({
	                    style : {
	                        pointList :link.pointList,
				...
				// 自定义事件
                if(typeof(link.onmouseover) !== 'undefined'){
                    if(link.onmouseover) {
                    	linkShape.onmouseover = link.onmouseover;
                    }
                }
                ...
                if(typeof(link.onclick) !== 'undefined'){
                    if(link.onclick) {
                    	linkShape.onclick = link.onclick;
                    }
                }
