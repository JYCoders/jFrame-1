...
for(var i=0; i<nodes.length; i++) {
	//$(nodes[i].target).find(".tree-node").removeClass("tree-node-selected");
	$(nodes[i].target).find("div.tree-node").addClass("tree-node-selected");
}
}else{
...
$("<a href=\"javascript:void(0)\"  class=\"datebox-clean\"></a>").html(opts.cleanText).appendTo(_791);
$("<a href=\"javascript:void(0)\" class=\"datebox-close\"></a>").html(opts.closeText).appendTo(_791);
_791.find(".datebox-current,.datebox-clean,.datebox-close").hover(function(){
...
_791.find(".datebox-clean").click(function(){
	$(_78c).combo("setValue","").combo("setText","");//设置空值 
	$(_78c).combo("hidePanel");//点击清空按钮之后关闭日期选择面板 
	});
...
//--------------
//扩展Easyui没有数据的时候显示没有相关记录
$.orzView = $.extend({}, $.fn.datagrid.defaults.view, {
	onAfterRender : function(target) {
		...
		if (!$(target).datagrid('getRows').length) {
			var d = $('<div class="datagrid-empty"></div>').html(
					opts.emptyMsg || '没有相关记录').appendTo(vc);
			d.css({
				position : 'absolute',
				'left' : 0,
				'top' : 32,
...
//扩展点击文本框也显示下拉面板的功能 
$.txtblur = $.extend($.fn.combo.methods, {   
    /**
     * 激活点击文本框也显示下拉面板的功能  
     * @param {Object} jq  
     */  
